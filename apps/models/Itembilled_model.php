...
class Itembilled_model extends My_Model {
	protected $_table = 'item_billed';
	...
	/**
	 * Get all list of item_billed which are in same company and not deleted (soft deleted) 
	 * @access public
	 * @return array
	 */
	public function get_list_all($project_id=null){
		...
		if ($project_id) {
			$this->db->select('items.*, '.$this->_table.'.billed, projects.name as project_name, projects.project_id');
			...
			$this->db->select('items.*, SUM('.$this->_table.'.billed) as billed, projects.name as project_name, projects.project_id');
	...
	/**
	 * Get an item info with billed details 
	 * @access public
	 * @return array
	 */
	public function get_item($item_id, $project_id){
		...
		$this->db->select('items.*, COALESCE('.$this->_table.'.billed , 0) AS billed, projects.name as project_name, projects.project_id');
		$this->db->join($this->_table, $this->_table.'.item_id=items.item_id AND '.$this->_table.'.project_id='.$project_id, 'left');
		$this->db->join('projects', 'projects.project_id='.$project_id, 'left');
	...
	public function updateBilled($item_id, $project_id, $quantity){
		$row = parent::get_by(array('item_id'=> $item_id, 'project_id'=>$project_id));
		if (empty($row)) {
			$this->insert(array('item_id'=> $item_id, 'project_id'=>$project_id, 'billed'=>$quantity));
		} else {
			$this->db->set('billed', 'billed+'.$quantity, FALSE);
