...
class Purchase_model extends My_Model {
	 ...
	 * @return double
	 */
	public function get_total_bill($project_id=null, $item_id=null, $supplier_id=null, $where=array(), $order_by='code', $order='desc'){
		$where['company_id'] = $this->session->userdata('company_id');
		$where['deleted'] = 0;
		if ($project_id) {
			$where['project_id'] = $project_id;
	 ...
	 * @return double
	 */
	public function get_total_payable($project_id=null, $item_id=null, $supplier_id=null, $where=array(), $order_by='code', $order='desc'){
		$where['company_id'] = $this->session->userdata('company_id');
		$where['deleted'] = 0;
		if ($project_id) {
			$where['project_id'] = $project_id;
		...
		$this->db->select('SUM(payable_amount) as total');
		$this->db->where($where);
		return $this->db->get($this->_table)->row()->total;		
	}
	 ...
	 * @return double
	 */
	public function get_total_security_payable($project_id=null, $item_id=null, $supplier_id=null, $where=array(), $order_by='code', $order='desc'){
		$where['company_id'] = $this->session->userdata('company_id');
		$where['deleted'] = 0;
		if ($project_id) {
			$where['project_id'] = $project_id;
